...
async function apiRequest(endpoint, method = 'GET', body = null) {
    ...
    try {
        ...
        log('TIME', `${method} ${endpoint} (${response.status}) took ${duration}ms`, 'TIME');
        ...
        const duration = Date.now() - startTime;
        log('NETWORK', `Failed to connect to ${BASE_URL}${endpoint} after ${duration}ms`, 'FAIL');
        console.error(err);
        process.exit(1);
    }
}
...
async function testProfile() {
    ...
    if (profileRes.success && profileRes.data.name) {
        ...
        log('Profile', `Failed to fetch profile: ${JSON.stringify(profileRes.error)}`, 'FAIL');
    }
...
async function testStandardResource(resourceName, endpoint, createPayload, updatePayload) {
    ...
    if (createRes.success && createRes.data.id) {
        ...
        log(`${resourceName} Create`, `Failed: ${JSON.stringify(createRes.error)}`, 'FAIL');
    ...
    if (getRes.success && Array.isArray(getRes.data)) {
        ...
        if (found) {
            ...
            log(`${resourceName} Read`, `Item ID ${createdId} not found in list`, 'FAIL');
        }
    } else {
        log(`${resourceName} Read`, `Failed to fetch list: ${JSON.stringify(getRes.error)}`, 'FAIL');
    }

    // 3. Update (PATCH)
    const patchRes = await apiRequest(`${endpoint}/${createdId}`, 'PATCH', updatePayload);
    if (patchRes.success) {
        // Verify update logic (simple check)
        const keys = Object.keys(updatePayload);
        const check = keys.every(key => patchRes.data[key] === updatePayload[key]);
        if(check) {
        ...
        log(`${resourceName} Update`, `Failed: ${JSON.stringify(patchRes.error)}`, 'FAIL');
    }
    ...
    if (deleteRes.success && deleteRes.status === 204) {
        ...
        log(`${resourceName} Delete`, `Failed: ${JSON.stringify(deleteRes.error)}`, 'FAIL');
    }
}
...
async function testCustomCategories() {
    ...
    if (catRes.success && catRes.data.id) {
        ...
        log('Category Create', `Created Category: ${catRes.data.categoryName} (${catId})`, 'PASS');
    } else {
        log('Category Create', `Failed to create category: ${JSON.stringify(catRes.error)}`, 'FAIL');
        return;
    }
    ...
    if (listRes.success && listRes.data.some(c => c.id === catId)) {
        ...
        log('Category List', `Category not found or request failed: ${JSON.stringify(listRes.error || 'N/A')}`, 'FAIL');
    }

    // 3. Add Item to Category
    const itemPayload = { datetime: new Date().toISOString(), note: "Morning Meditation" };
    const addItemRes = await apiRequest(`/category/${catId}/add`, 'POST', itemPayload);
    
    if (addItemRes.success && addItemRes.data.id) {
        itemId = addItemRes.data.id;
        log('Category Item Add', `Added Item ID: ${itemId}`, 'PASS');
    } else {
        log('Category Item Add', `Failed to add item: ${JSON.stringify(addItemRes.error)}`, 'FAIL');
        return;
    }
    ...
    if (listItemRes.success && listItemRes.data.some(i => i.id === itemId)) {
        ...
        log('Category Item List', `Item not found or request failed: ${JSON.stringify(listItemRes.error || 'N/A')}`, 'FAIL');
    }

    // 5. Update Item
    const updateItemPayload = { note: "Evening Meditation" };
    const patchItemRes = await apiRequest(`/category/${catId}/${itemId}`, 'PATCH', updateItemPayload);
    if (patchItemRes.success && patchItemRes.data.note === "Evening Meditation") {
        log('Category Item Update', 'Item updated successfully', 'PASS');
    } else {
        log('Category Item Update', `Failed to update item: ${JSON.stringify(patchItemRes.error)}`, 'FAIL');
    }
    ...
    if (delItemRes.success) {
        ...
        log('Category Item Delete', `Failed to delete item: ${JSON.stringify(delItemRes.error)}`, 'FAIL');
    }
}
